function race(input) {
  ...
  while(currentRow !== 'end of race') {
    ...
    let distance = currentRow.match(digitsPattern).reduce((prev, cur) => prev + Number(cur), 0)

    if(objOfParticipants[currentName] !== undefined) {
      objOfParticipants[currentName] += distance
    }
